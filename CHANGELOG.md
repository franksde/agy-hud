...
- `quota refresh` names the cause when the loopback server rejects the probe as unauthenticated. On Antigravity CLI 1.2.11, and possibly earlier 1.2.x releases, the `agy` server answers `GetUserStatus` with `401 missing CSRF token` and the status line is not given that token, so the probe cannot refresh; it used to report a generic "Failed to query GetUserStatus". The HUD falls back to the quota in the status-line payload, and it records the rejection against the CLI `version` from the payload in `quota_cache.json.auth-rejected.json`: while that version keeps running, neither the same-frame nor the background probe runs, which saves the 0.2–0.5 s a failing probe added to the redraw at the end of every turn. A CLI update retries once, a success clears the record, and a manual `quota refresh` always probes. Both READMEs state this, and verified compatibility is now through 1.2.11 for install, wiring and rendering.
